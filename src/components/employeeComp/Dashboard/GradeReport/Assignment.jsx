...
const Assignment = ({ openAssignment, AssignmentModalHandler, courseName }) => {
      ...
      `}
        >
         ...
         ${maxScreen ? "h-[15%]" : "h-[20%] rounded-t-[1vw]"
            ...
            >
                <div>
                    <h1 className="text-[2vw] font-semibold">Assignments Detail</h1>
                </div>
                <div className="flex gap-x-[1vw]">
                    ...
                    <button className="w-[1.2vw] h-[1.2vw] hover:scale-110 duration-300" onClick={() => AssignmentModalHandler("No Data")}>
        ...
        <tbody>
          {AssignmentsVal.map((assignment, index) => (
            <tr key={index}
            className={`${assignment.status === "Submitted" ? "bg-green-400" :
            assignment.status === "Late Submitted" ? "bg-yellow-400" :
            assignment.status === "Not Submitted" ? "bg-red-400" :
             "bg-gray-100"}`}>
              <td className="px-4 py-2 border-b text-start">{assignment.assignment}</td>
